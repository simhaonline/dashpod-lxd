...
$repo_url = filter_var(urldecode($_GET['repo']), FILTER_SANITIZE_STRING);

#Get the JSON data
$results = exec("sudo lxc image list $repo: --project $project --format json 2>&1", $output, $return);
...
if ($return == 0 ) {
  ...
  foreach ($items as $item) {
    ...
    echo "<td>";
      ...
      echo '<a class="dropdown-item" href="./php/lxd/image-copy.php?image='. $fingerprint . '&remote=' . $remote_url . '&project=' . $project_url . '&repo=' . $repo_url . '">Download</a>';
